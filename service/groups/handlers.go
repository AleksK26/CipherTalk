...
func AddToGroupHandler(c *gin.Context) {
	...
	var user db.User
	if err := db.DB.First(&user, "id = ?", request.UserID).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "User not found"})
		return
	}

	db.DB.Model(&group).Association("Members").Append(&user)
...
func LeaveGroupHandler(c *gin.Context) {
	...
	db.DB.Model(&group).Association("Members").Delete(&user)
